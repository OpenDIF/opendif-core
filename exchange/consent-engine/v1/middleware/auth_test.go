...
func createTestJWTVerifier(t *testing.T, privateKey *rsa.PrivateKey, issuer, audience string) *auth.JWTVerifier {
	...
	// Wait for initial JWKS fetch to complete (with timeout)
	ready := verifier.WaitForJWKS(5 * time.Second)
	require.True(t, ready, "JWKS should be loaded within 5 seconds")
